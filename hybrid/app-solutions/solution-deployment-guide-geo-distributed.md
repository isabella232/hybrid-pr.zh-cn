...
2. 添加一条 CNAME 记录来将子域映射到应用的默认主机名。
   ...
   对于 www \. northwindcloud.com 域示例，添加将名称映射到的 CNAME 记录 `<app_name>.azurewebsites.net` 。
...
私钥文件是通过 OpenSSL 创建的。 若要将证书导出为 PFX，请运行以下命令，并将占位符 `<private-key-file>` 和 `<merged-certificate-file>` 私钥路径以及合并的证书文件一起替换：
...
在不同的浏览器中，请参阅以 `https://<your.custom.domain>` 确保 web 应用服务。
